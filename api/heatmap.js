...
export default async function handler(req, res){
  ...
  try{
    ...
    // streak with allowed days (Mon..Sun set), "continuesToday" flag
    const allowed = (habit.streak_days || ['Mon','Tue','Wed','Thu','Fri','Sat','Sun']);
    const dow = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];

    let streak = 0;
    let continuesToday = false;
    // compute pointer at yesterday if today missing but allowed
    ...
    let i = lastIdx;
    if (todayAllowed && todayValue===0){
      // streak would continue today if yesterday or earlier have positive chain
      continuesToday = true;
      i--;
    }
    for (; i>=0; i--){
    ...
    return ok(res, {
      ...
      habit: { id: habit.id, name: habit.name, type: habit.type, unit: habit.unit, colorHex: habit.color_hex, streakDays: habit.streak_days },
